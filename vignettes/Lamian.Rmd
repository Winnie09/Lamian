...
Further information can be found in our paper describing the **Lamian** package:

- Hou, W., Ji, Z., Chen, Z., Wherry, E.J., Hicks, S.*, and Ji, H.* A statistical framework for differential pseudotime analysis with multiple single-cell RNA-seq samples. [Nature Communications 14, 7286 (2023)](https://www.nature.com/articles/s41467-023-42841-y).
...
To save the gene expression in hdf5 file format, we apply the following codes which creates a new file **multi.h5** (we deposited this file *multi.h5* in the package folder *data/* )
```{r eval=FALSE}
if (requireNamespace("rhdf5", quietly = TRUE)) rhdf5::h5closeAll()
...
```{r}
suppressMessages(library(slingshot)) ## install it from Bioconductor if encounter an R version issue
...
system.time({
  sce <- slingshot::slingshot(sce, clusterLabels = 'GMM', reducedDim = 'PCA')
})
```
...
plot(reducedDims(sce)$PCA, 
     ...
     xlab = 'Principal component 1', 
     ylab = 'Principal component 2')
lines(slingshot::SlingshotDataSet(sce)@curves[[1]], lwd=2, col='black')
```
...
## Important: A vector with names is easier to handle in subsetting. 
names(pt) <- colnames(cnt)
```
...
## Important: remove pseudotime that is NA 
pt <- pt[!is.na(pt)] 
