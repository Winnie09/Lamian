...
evaluate_uncertainty <- function(inferobj, n.permute, subset.cell = NULL){
  ...
  for (pmid in seq(1, n.permute)){
    ...
    if (exists('ord.pm')) {rm('ord.pm')} 
    tryCatch({ord.pm <- TSCANorder(mcl.pm, startcluster = as.numeric(start.cluster), listbranch = T,orderonly = T)},error=function(e) {})
    if (!exists('ord.pm')) {
      ord.pm <- TSCANorder(mcl.pm, startcluster = as.numeric(start.cluster), listbranch = F,orderonly = T)
    }
    # str(ord.pm)
