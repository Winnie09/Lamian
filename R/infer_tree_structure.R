...
#' @return a list
#' @export
#' @import TSCAN   scattermore RColorBrewer igraph
...
infer_tree_structure <- function(pca, cellanno, expression, origin.marker=NA, origin.celltype=NA,   number.cluster = NA, plotdir =NA, xlab = 'PC1', ylab = 'PC2', max.clunum=50){
  alls <- cellanno[,2]
  ...
  if (!is.na(origin.celltype)){
    pd = data.frame(x = pr[,1], y = pr[,2], clu = as.factor(mcl$clusterid), celltype = cellanno[match(rownames(pr), cellanno[,1]),3], stringsAsFactors = F)
    tab <- table(pd[,3:4])
    tab <- tab/rowSums(tab)
    pd <- melt(tab)
    ...
    pd <- data.frame(x = pr[,1], y = pr[,2], clu = as.factor(mcl$clusterid), mark = colMeans(expression[origin.marker, , drop = FALSE]), stringsAsFactors = F)
