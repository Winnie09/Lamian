...
cell_prop_test <- function(cellanno, pseudotime, design=NULL, permuiter=100, EMmaxiter=100, ncores=detectCores()) {
  ...
  res <- lamian.test(expr=ptexpr, cellanno=data.frame(cell=ptdat$cell,sample=ptdat$s), pseudotime=ptpt, design=design, ncores=ncores, test.type = 'Variable', demean = FALSE, test.method = 'permutation', ncores.fit = 1,fix.all.zero=F, permuiter = permuiter, EMmaxiter=EMmaxiter)
