...
plotGeneCellAndPopulation <- function(testobj,
  ...
  if (toupper(type) == 'TIME'){
    ...
    if (is.na(gene[1])) gene <- rownames(fit[[1]])
    ...
    pd <- data.frame(pd[, c('gene', 'pseudotime', 'expression')], type = paste0(colnames(design)[2], '_', pd[,'Variable']))
    p <- ggplot() + 
      geom_point(data= pd, aes(x = pseudotime, y = expression, group = type, color = type), size = point.size, alpha = point.alpha) + 
      geom_line(data= ld, aes(x = pseudotime, y = expression, group = type, color = type), size = line.size) +
      theme_classic() +
      xlab('Pseudotime') +
      ylab('Expression') +
      labs(color = '') 
