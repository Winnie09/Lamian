...
plotClusterMeanAndDiff <- function(testobj,
  ...
  if (length(unique(pd$type)) < 8) {
    ...
    p1 <-
      p1 + scale_color_manual(values = colorRampPalette(brewer.pal(8, 'Dark2'))(length(unique(pd$type))))
  }
  ...
  if (length(unique(pd$cluster)) < 8) {
    ...
    p2 <-
      p2 + scale_color_manual(values = colorRampPalette(brewer.pal(8, 'Dark2'))(length(unique(pd$cluster))))
  }
